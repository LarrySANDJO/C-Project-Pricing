...
/* =========================================================
   LEISEN-REIMER (LR)
   Amélioration de la convergence pour les options européennes
   ========================================================= */
void BinomialTreePricer::compute_lr_parameters()
{
    double K = option_.payoff().strike();
    double T = option_.maturity();
    
    // Calcul de d1 et d2 (Black-Scholes)
    double d1 = (std::log(S0_ / K) + (b_ + 0.5 * sigma_ * sigma_) * T)
                / (sigma_ * std::sqrt(T));
    double d2 = d1 - sigma_ * std::sqrt(T);
    
    // Inversion de Peizer-Pratt
    double p_up = h_inverse(d1, static_cast<int>(N_));
    double p_down = h_inverse(d2, static_cast<int>(N_));
    
    // Facteurs u et d
    double growth = std::exp(b_ * dt_);
    u_ = growth * p_down / p_up;
    d_ = (growth - p_up * u_) / (1.0 - p_up);
    
    p_ = p_up;
    df_ = std::exp(-r_ * dt_);
}

// Fonction h de Peizer-Pratt
double BinomialTreePricer::h_function(double x) const
{
    return 0.5 + 0.5 * std::erf(x / std::sqrt(2.0));
}

// Inversion de h par méthode de Peizer-Pratt
double BinomialTreePricer::h_inverse(double z, int n) const
{
    if (n <= 0) return 0.5;
    
    double x = z / (n + 1.0/3.0 + 0.1/(n+1.0));
    x = x / std::sqrt(1.0 + x*x/(n + 1.0/6.0));
