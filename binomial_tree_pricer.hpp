...
public:
    enum class TreeType
    {
        ...
        LeisenReimer        // Arbre LR (amélioration convergence)
    };
...
private:
    ...
    // LR : Leisen-Reimer (convergence améliorée)
    void compute_lr_parameters();
    
    // Construction et évaluation de l'arbre
    double evaluate_tree() const;
    
    // Helper pour l'arbre LR
    double h_inverse(double z, int n) const;
    double h_function(double x) const;
